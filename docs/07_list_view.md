...
- [ ] Add `useInfiniteQuery` hook wrapping `getShows` (`src/api/shows.ts`), keyed by page, `getNextPageParam` incrementing the page number
- [ ] Wire `(tabs)/index.tsx` to the hook, replacing `MOCK_SHOWS`; trigger `fetchNextPage` via `FlatList`'s `onEndReached`
